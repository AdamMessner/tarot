...
const build = (options) => (env, argv) => {
	...
	const source = path.resolve(cwd, options?.source || 'src')
	const output = path.resolve(cwd, options?.output || 'dist')
			...
			rules: [
				{
					...
					include: [source, ...(options?.babelIncludes || [])],
					use: {
						loader: 'babel-loader',
					},
				},
	...
	return Object.entries(options?.entries || {}).map(
